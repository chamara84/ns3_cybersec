...
NS_OBJECT_ENSURE_REGISTERED (Icmpv4RedirectionHeader);

TypeId
Icmpv4RedirectionHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Icmpv4RedirectionHeader")
    .SetParent<Header> ()
    .SetGroupName ("Internet")
    .AddConstructor<Icmpv4RedirectionHeader> ()
  ;
  return tid;
}

void
Icmpv4RedirectionHeader::SetData (Ptr<const Packet> data)
{
  NS_LOG_FUNCTION (this << *data);
...
uint32_t
Icmpv4RedirectionHeader::GetData (uint8_t * payload) const
{
  NS_LOG_FUNCTION (this << payload);
  memcpy (payload, m_data, m_dataSize);
  return m_dataSize;
...
void
Icmpv4RedirectionHeader::SetType (uint8_t type)
{
  NS_LOG_FUNCTION (this << static_cast<uint32_t> (type));
  m_type = type;
}
void
Icmpv4RedirectionHeader::SetCode (uint8_t code)
{
  NS_LOG_FUNCTION (this << static_cast<uint32_t> (code));
  m_code = code;
}

Icmpv4RedirectionHeader::Icmpv4RedirectionHeader ()
  : m_identifier (0),
    m_sequence (0),
    m_dataSize (0),
	m_type(5),
...
void
Icmpv4RedirectionHeader::EnableChecksum (void)
{
  NS_LOG_FUNCTION (this);
  m_calcChecksum = true;
}
...
TypeId
Icmpv4RedirectionHeader::GetInstanceTypeId (void) const
{
  NS_LOG_FUNCTION (this);
  return GetTypeId ();
}
...
uint32_t
Icmpv4RedirectionHeader::GetDataSize (void) const
{
  NS_LOG_FUNCTION (this);
  return m_dataSize;
}
uint32_t
Icmpv4RedirectionHeader::GetSerializedSize (void) const
{
  NS_LOG_FUNCTION (this);
  return 4 + m_dataSize;
}
void
Icmpv4RedirectionHeader::Serialize (Buffer::Iterator start) const
{
  NS_LOG_FUNCTION (this << &start);
  start.WriteHtonU16 (m_identifier);
  start.WriteHtonU16 (m_sequence);
...
void
Icmpv4RedirectionHeader::SetHeader (Ipv4Header header)
{
...
uint32_t
Icmpv4RedirectionHeader::Deserialize (Buffer::Iterator start)
{
	NS_LOG_FUNCTION (this << &start);
...
void
Icmpv4RedirectionHeader::Print (std::ostream &os) const
{
  NS_LOG_FUNCTION (this << &os);
  os << "identifier=" << m_identifier << ", sequence="  << m_sequence << ", data size=" << m_dataSize;
}
