                           ...
                           const Address &toAddress)
{
  ...
  NS_LOG_FUNCTION (this << "NetDevice"<<m_boundnetdevice );
  if (m_boundnetdevice)
    {
  ...
  if (dst.IsBroadcast () || subnetDirectedBroadcast)
    {
      ...
      if (boundNetDevice == 0)
        {
          NS_LOG_DEBUG ("dropped because no outgoing Broadcast route.");
          return -1;
        }
      ...
      if (!oif && src != Ipv4Address::GetAny ())
        {
          ...
          NS_LOG_LOGIC ("Set index " << oif << "from source " << src <<"to dst"<<dst);
        }
