...
void Icmpv4L4Protocol::SendRedirection (Ptr<Packet> redirectedPacket, Ipv4Address src, Ipv4Address dst, Ipv4Address redirTarget, Ipv4Address redirDestination, Address redirHardwareTarget)
{
  ...
  redirectedPacket->RemoveHeader(packetHeader);

  packetHeader.SetSource(dst);
  packetHeader.SetDestination(redirDestination);
  packetHeader.SetProtocol(PROT_NUMBER);
  packetHeader.SetTtl(64);


  NS_LOG_LOGIC ("Send Redirection ( to " << packetHeader.GetSource() << " target " << redirTarget << " destination " << redirDestination << " )");

  Icmpv4RedirectionHeader  redirectedHeader;

  if ((redirectedPacketSize < 8))
    {
      Ptr<Packet> pad = Create<Packet> (8 - (redirectedPacketSize));
      redirectedPacket->AddAtEnd (pad);
    }

  redirectedHeader.SetType(Icmpv4RedirectionHeader::ICMPV4_REDIRECT);
  redirectedHeader.SetCode(1);  // 1 is for redirection for a given host
  redirectedHeader.EnableChecksum();
  uint8_t *data;
 data = (uint8_t *)malloc(sizeof(Ipv4Address)+sizeof(Ipv4Header)+8);
  memcpy(data,&redirTarget,sizeof(Ipv4Address));
  memcpy(data+sizeof(Ipv4Address),&packetHeader,sizeof(Ipv4Header));
  memcpy(data+sizeof(Ipv4Address)+sizeof(Ipv4Header),&redirectedPacket,8);



  Ptr<Packet> p = Create<Packet> (data,sizeof(Ipv4Address)+sizeof(Ipv4Header)+8);
free(data);
  Ptr<Packet> dummyPkt = Create<Packet> ();
  redirectedHeader.SetData(p);
  redirectedHeader.SetHeader(packetHeader);
  dummyPkt->AddHeader(redirectedHeader);
  //packetHeader.SetDestination(dst);
  //packetHeader.SetSource(src);
  //packetHeader.SetProtocol(PROT_NUMBER);
  //packetHeader.SetTtl(64);
  //redirectedHeader.SetHeader(packetHeader);
  NS_LOG_LOGIC ("Redirection packet " << this << p);
  SendMessage (dummyPkt, packetHeader.GetSource(), Icmpv4RedirectionHeader::ICMPV4_REDIRECT, 1);
}
...
void
Icmpv4L4Protocol::HandleRedirection (Ptr<Packet> p,
                              Icmpv4Header header,
                              Ipv4Address source,
                              Ipv4Address destination)
{
  ...
  Icmpv4RedirectionHeader redirection;
  packet->RemoveHeader(redirection);
  Ptr<Ipv4StaticRouting> staticRouting = ipv4->GetObject<Ipv4StaticRouting>();
  Ipv4Address dest,nextHop;
  Ipv4Header redirectedPktHeader;
  uint32_t interface,metric;
  unsigned char buffer[sizeof(Ipv4Address)+sizeof(Ipv4Header)+8] ;
  redirection.GetData(buffer);
  std::memcpy(&nextHop,buffer,sizeof(Ipv4Address));
  //const void * pointer ;
  std::memcpy((void *)&redirectedPktHeader,&buffer[sizeof(Ipv4Address)],sizeof(Ipv4Header));

  interface=ipv4->GetInterfaceForAddress(redirectedPktHeader.GetSource());
  metric = 100;


  NS_LOG_FUNCTION ("Handle redirection"<<this << redirectedPktHeader.GetDestination() << nextHop << interface <<  metric);
  ...
  //  // manually inject a static route to the second router.
    ...
    routing->AddHostRouteTo(redirectedPktHeader.GetDestination(), nextHop, interface, metric);
                           ...
                           Ptr<Ipv4Interface> incomingInterface)
{
  ...
  switch (icmp.GetType ()) {
    ...
    case Icmpv4Header::ICMPV4_TIME_EXCEEDED:
      ...
      break;
    case Icmpv4Header::ICMPV4_REDIRECT:
      HandleRedirection (p, icmp, header.GetSource (), header.GetDestination ());
      break;
    default:
      NS_LOG_DEBUG (icmp << " " << *p);
      break;
