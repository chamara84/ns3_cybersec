...
void MyApp::StartApplication (void)
{
 ...
 // m_type_of_node = 1 ingress 2 egress 3 man-in-the-middle 4 just_forward
	Ptr<SocketFactory> rxSocketFactory = this->node->GetObject<Ipv4RawSocketFactory> ();
	...
	m_socket = DynamicCast<Ipv4RawSocketImpl>(rxSocketFactory->CreateSocket ());
//		//m_rsocket->SetRecvPktInfo (true);
		m_socket->SetProtocol(TcpL4Protocol::PROT_NUMBER);
...
//
	if (m_type_of_node == 2)
	{
		m_socket->Bind (InetSocketAddress (Ipv4Address("172.24.2.29"), m_port));
		int interfaceIndex = ipV4Info->GetInterfaceForAddress(Ipv4Address("172.24.2.29"));
		std::cout<<"NetDev:"<<interfaceIndex;
...
//	}
	//else
		m_rsocket->Bind (InetSocketAddress (Ipv4Address::GetAny(), m_port));
...
//		   packet->RemoveHeader (udpHdr1);
	       ...
	       ipV4Hdr.SetSource(Ipv4Address("172.24.2.29"));
...
MyApp::pktProcessingIngressNode (Ptr<Socket> socket)
 {
	 Packet::EnablePrinting();
	   ...
	   Ptr<Socket>  socketSend = Socket::CreateSocket (GetNode (), tid);

	   socketSend->Bind ();
	   socketSend->Connect (InetSocketAddress (Ipv4Address("172.24.9.250"), 7010));
...
//		   ipV4Hdr1.Print(std::cout);
	...
	//	   TcpHeader udpHdr1;
	//	   packet->RemoveHeader (udpHdr1);
	//	   udpHdr1.InitializeChecksum(ipV4Hdr1.GetSource(), ipV4Hdr1.GetDestination(), ipV4Hdr1.GetProtocol());
//		   printf("UDP HDR Ingress \n");
//		   udpHdr1.Print(std::cout);
		//   printf("\n");
//		   if(udpHdr1.GetFlags() == TcpHeader::SYN)
//		   		   		   {
//		   		   			   std::cout << "Flags" << udpHdr1.FlagsToString(udpHdr1.GetFlags(), ",") <<endl;
//		   		   			Ptr<Ipv4L3Protocol> ipv4 = this->node->GetObject<Ipv4L3Protocol> ();
//		   		   			int int_id = ipv4->GetInterfaceForDevice(socket->GetBoundNetDevice());
//		   		   		Ptr<TcpL4Protocol> tcp = this->node->GetObject <TcpL4Protocol> ();
//		   		   		udpHdr1.InitializeChecksum(ipV4Hdr1.GetSource(), ipV4Hdr1.GetDestination(), ipV4Hdr1.GetProtocol());
//		   		   		packet->AddHeader(udpHdr1);
//		   		   		 // tcp->Receive (packet, ipV4Hdr1, ipv4->GetInterface(int_id));
//
//
//		   		   		   }
...
//		   				   		   			}
		   				   		   	TcpHeader udpHdr;
		   				   		   				  		   		Ipv4Header ipV4Hdr;
		   				   		   				  		   		ipV4Hdr.SetSource(Ipv4Address("172.24.2.155"));
		   				   		   				  		   		ipV4Hdr.SetProtocol(17); // 17 stands for udp
		   				   		   				  		   		ipV4Hdr.SetTtl(255);
		   				   		   				  		   		udpHdr.SetDestinationPort(7001);
		   				   		   				  		   		udpHdr.SetSourcePort(7001);
...
//	       std::cout <<"Index of 2 Node:"<<secondNodeIndex<<endl;
	       if(type==5 || type==8)
	       {
	    	   ...
	    	   ipV4Hdr.SetIdentification(m_packetsSent);
	    	   //udpHdr.ForcePayloadSize(packet->GetSize ());
	    	   udpHdr.InitializeChecksum(Ipv4Address("172.24.2.155"),Ipv4Address(to.c_str ()),17);
...
static void
    pktProcessingIngressNode (Ptr<Node> n,Ptr<Socket> socket)
 {
	 Packet::EnablePrinting();
	   Ptr<Packet> packet;
	   TypeId tid = TypeId::LookupByName ("ns3::TcpSocketFactory");
	   Ptr<Socket>  socketSend = Socket::CreateSocket (n, tid);
	   socketSend->Bind ();
	   socketSend->Connect (InetSocketAddress (Ipv4Address("172.24.9.250"), 7010));
...
//		   udpHdr.SetSourcePort(7001);
	       ...
	       int type;
	      int indexOfNode;
	       memcpy(&bufferFloat, &buffer[0], 28);

	       bufferFloat[0] = ntohl(bufferFloat[0]);
	       bufferFloat[1] = ntohl(bufferFloat[1]);
	       bufferFloat[3] = ntohl(bufferFloat[3]);
	       bufferFloat[4] = ntohl(bufferFloat[4]);
	       bufferFloat[5] = ntohl(bufferFloat[5]);
	       bufferFloat[6] = ntohl(bufferFloat[6]);
	       memcpy(&data[0],&bufferFloat[3],4);
	       memcpy(&data[1],&bufferFloat[4],4);
	       memcpy(&data[2],&bufferFloat[5],4);
	       memcpy(&data[3],&bufferFloat[6],4);

	       type = bufferFloat[0];
	       indexOfNode = bufferFloat[1];
	       std::cout <<"Type:"<<type<<std::endl;
	       std::cout <<"Index of Node:"<<indexOfNode<<std::endl;
	       std::cout <<"Flexibility:"<<data[0]<<data[1]<<data[2]<<data[3]<<std::endl;
	       std::string to;
	       if(type==5 || type==8)
	       {
	    	   switch(indexOfNode)
...
//       ipv4.Assign (NetDevn2Attacker4);
   ...
   //setting up ingress node to communicate with the GTNET
       ...
       address.SetBase ("172.24.0.0", "255.255.0.0", "0.0.2.155");
...
//        ipv4.Assign (NetDevDER4Egress);
        ...
        //Second Emulated interface Egress node
             std::string device2Name ("p24p1"); //edit the name corresponding to the device name
             ...
             ipv4.SetBase ("172.24.0.0", "255.255.0.0", "0.0.2.29");
...
//       rxSocketn0->Bind (InetSocketAddress (Ipv4Address ("172.24.2.55"), 4888));
       ...
       staticRoutingIngressNode->SetDefaultRoute(Ipv4Address("172.24.0.1"),ipv4ingressNode->GetInterfaceForAddress(Ipv4Address("172.24.2.155")),0); //only for testing interface is one because emulation was the first device installed
...
//
       ...
       Ptr<Ipv4> ipv4Aggregator = Aggregator->GetObject<Ipv4>();
              ...
              staticRouting_Aggregator->AddHostRouteTo (Ipv4Address ("172.24.2.29"), Ipv4Address ("10.1.8.6"), 2);
...
//                  appTcp->SetStopTime (Seconds (stopTime));
       ...
       app->SetAttribute("Local",Ipv4AddressValue("172.24.2.155"));
       app->SetAttribute("Port", UintegerValue (7001));
       MyApp appTcp ;
       Callback<void, Ptr<Socket>> recvCb;
       recvCb = MakeBoundCallback (&pktProcessingIngressNode,ingressNode);
       app->SetRcvDataCb( recvCb);

       ingressNode->AddApplication (app);
       app->SetStartTime (Seconds (1.));
       app->SetStopTime (Seconds (stopTime));

//       Ptr<MyApp> appTcp = CreateObject<MyApp> (); //modify
//              appTcp->Setup (ingressNode,Ipv4Address ("172.24.2.55"),Ipv4Address ("10.1.8.6"), 7001,7001,INGRESS_NODE);
//              ingressNode->AddApplication (appTcp);
//              app->SetStartTime (Seconds (1.));
//              app->SetStopTime (Seconds (stopTime));
...
//
//       TypeId tid = TypeId::LookupByName ("ns3::TcpSocketFactory");
//       		Ptr<Socket> Tcp_socket = Socket::CreateSocket (ingressNode, tid);
//       		InetSocketAddress local = InetSocketAddress (Ipv4Address ("172.24.2.55"), 7001);
...
//       		Tcp_socket->Listen();
       ...
       appDER1Extern->Setup (DERs.Get(0),Ipv4Address ("10.1.6.2"),Ipv4Address ("172.24.2.29"), 7001,7001,FORWARDING_NODE);
       ...
       appDER2Extern->Setup (DERs.Get(1),Ipv4Address ("10.1.6.6"),Ipv4Address ("172.24.2.29"), 7001,7001,FORWARDING_NODE);
       ...
       appDER3Extern->Setup (DERs.Get(2),Ipv4Address ("10.1.6.10"),Ipv4Address ("172.24.2.29"), 7001,7001,FORWARDING_NODE);
       ...
       appDER4Extern->Setup (DERs.Get(3),Ipv4Address ("10.1.6.14"),Ipv4Address ("172.24.2.29"), 7001,7001,FORWARDING_NODE);
...
//Setting up the egress node
       ...
       Ptr<MyApp> app2 = CreateObject<MyApp> ();
               app2->Setup (egressNode,Ipv4Address ("172.24.2.29"),Ipv4Address ("172.24.9.240"), 7001,7001,EGRESS_NODE);
