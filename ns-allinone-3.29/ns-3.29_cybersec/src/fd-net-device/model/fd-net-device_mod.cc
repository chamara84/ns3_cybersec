...
FdNetDevice::ForwardUp (void)
{
  ...
  Ipv4Header iph;
  UdpHeader udpHeader;
  NS_LOG_LOGIC ("Size with IP: " << packet->ToString());
  packet->RemoveHeader(iph);
  NS_LOG_LOGIC ("Size with UDP: " << packet->ToString());
  packet->RemoveHeader(udpHeader);
  NS_LOG_LOGIC ("Size Data: " << packet->ToString());
  unsigned char buffer[packet->GetSize()] ;
  packet->CopyData(buffer, packet->GetSize ());

  NS_LOG_LOGIC ("Bufer: " << buffer);
...
//        //copy->RemoveHeader (iph);
      ...
      // NS_LOG_LOGIC ("Pkt dst port before mod is " << udpHeader.GetDestinationPort());
       NS_LOG_LOGIC ("Pkt src IP before mod is " << srcAddr);
       const Ipv4Address GTNET_low = Ipv4Address ("172.24.9.240");
       //const Ipv4Address GTNET_high = Ipv4Address ("172.24.9.254");
       bool isModified =false;
       Ptr<Packet> packetNew;

       if(srcAddr.IsEqual(GTNET_low))
       {
    	   Ptr<Ipv4L3Protocol> ipv4Proto;
    	   ipv4Proto = this->GetNode()->GetObject<Ipv4L3Protocol> ();

    	   unsigned char buffer[packet->GetSize()] ;
    	   unsigned int bufferFloat[packet->GetSize()];
    	   packet->CopyData (buffer, packet->GetSize ());
    	   NS_LOG_LOGIC ("Inside inequality");


    	          int integerData;
    	          float floatingPointData;
    	          memcpy(&bufferFloat, &buffer, 8);

    	          bufferFloat[0] = ntohl(bufferFloat[0]);
    	          bufferFloat[1] = ntohl(bufferFloat[1]);
    	          memcpy(&floatingPointData, &bufferFloat[1], 4);
    	          integerData =  bufferFloat[0];
    	          NS_LOG_LOGIC ("Inside inequality float data" << floatingPointData);
    	          NS_LOG_LOGIC ("Inside inequality int data" <<  integerData );
    	          memcpy(&floatingPointData, &bufferFloat[1], 4);
    	          ...
    	          packetNew = Create<Packet>(reinterpret_cast<const uint8_t*> (bufferFloat),8);
    	          packetNew->AddHeader(udpHeader);
    	          packetNew->AddHeader(iph);
    	          packetNew->PeekHeader(iph);
    	          packetNew->PeekHeader(udpHeader);
    	          NS_LOG_LOGIC ("PacketNew dst port after mod is " << udpHeader.GetDestinationPort());
    	          NS_LOG_LOGIC ("PacketNew IP after mod is " << iph.GetDestination());

    	          ipv4Proto->Receive(this,packet,iph.GetProtocol(),iph.GetSource(),iph.GetDestination(),packetType);
       ...
       packet->PeekHeader(iph);
       NS_LOG_LOGIC ("Packet dst IP after mod is " << iph.GetDestination());
       packet->PeekHeader(udpHeader);
       NS_LOG_LOGIC ("Packet dst port after mod is " << udpHeader.GetDestinationPort());
...
//        bufferFloat[1] = htonl(bufferFloat[1]);
//        //memcpy(&buffer, &bufferFloat, 8);
////        Ptr<Packet> packetNew = Create<Packet>(bufferFloat,8);
////        packetNew->AddHeader(udpHeader);
////        packetNew->AddHeader(iph);
////        NS_LOG_LOGIC ("Pkt source after mod is " << packetNew->ToString());
//
//
//        packet->AddHeader(udpHeader);
//        packet->AddHeader(iph);
        NS_LOG_LOGIC ("Pkt after mod is " << packet->ToString());
  //
