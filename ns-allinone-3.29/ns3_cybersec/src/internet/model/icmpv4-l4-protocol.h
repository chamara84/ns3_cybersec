...
public:
  ...
  /**
   * \brief Send a ICMP redirection message
   * \param redirectedPacket the redirected packet
   * \param src IPv4 address to send the redirect from
   * \param dst IPv4 address to send the redirect to
   * \param redirTarget IPv4 target address for Icmpv4Redirection (the new gateway)
   * \param redirDestination IPv4 destination address for Icmpv4Redirection (the destination to be rerouted)
   * \param redirHardwareTarget L2 target address for Icmpv6OptionRdirected
   */
  void SendRedirection (Ptr<Packet> redirectedPacket, Ipv4Address src, Ipv4Address dst, Ipv4Address redirTarget, Ipv4Address redirDestination, Address redirHardwareTarget);
...
private:
  ...
  /**
    * \brief Handles an incoming ICMP Rediection packet
    * \param p the packet
    * \param header the IP header
    * \param source the source address
    * \param destination the destination address
