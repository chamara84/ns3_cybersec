...
void Ipv4L3Protocol::SetIcmpV4Redirect(void)
{
	m_sendIcmpv4Redirect = true;
}
...
//  route = m_routingProtocol->RouteOutput(packet, ipHeader, oif, errno_);
//
//  if(m_sendIcmpv4Redirect && GetInterfaceForDevice (route->GetOutputDevice())==interface)
//        	  {
//        		  //the output interface is the same as the interface of the host who sent the packet.
//        		  //therefore a better route exists if the host send the packet directly to the dest of the rtentry
//        		  //send icmp redirect msg
//
//
//        		  NS_LOG_LOGIC ("send icmp redirect msg to << ipHeader.GetSource ()");
//        		GetIcmp()->SendRedirection(packet,route->GetSource() , ipHeader.GetSource (), route->GetGateway(), ipHeader.GetDestination (), route->GetOutputDevice()->GetAddress());
//
//        	  }
...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  Socket::SocketErrno errno_;
    ...
    reverseIpHeader.SetDestination(ipHeader.GetSource());
    ...
    if(m_sendIcmpv4Redirect && GetInterfaceForDevice (route->GetOutputDevice())==interface)
          	  {
          		  ...
          		  NS_LOG_LOGIC (this<<"send icmp redirect msg to "<< ipHeader.GetSource ()<<  rtentry->GetGateway()  << m_node->GetId ());
          		GetIcmp()->SendRedirection(packet,rtentry->GetSource() , ipHeader.GetSource (), rtentry->GetGateway(), ipHeader.GetDestination (), rtentry->GetOutputDevice()->GetAddress());
