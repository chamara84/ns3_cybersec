...
void MyApp::StartApplication (void)
{
 ...
 // m_type_of_node = 1 ingress 2 egress 3 man-in-the-middle 4 just_forward
	...
	m_rsocket->SetProtocol(UdpL4Protocol::PROT_NUMBER);
...
//                                       "PeakRate", DataRateValue (DataRate (DataRate ("1000Mbps"))));
      ...
      d0d2.Get(0)->GetAttribute ("TxQueue", ptr);
                        Ptr<Queue<Packet> > txQueuen01 = ptr.Get<Queue<Packet> > ();
                        txQueuen01->SetMaxSize( QueueSize (QueueSizeUnit::PACKETS, 10));
                        d0d2.Get(1)->GetAttribute ("TxQueue", ptr);
                                          Ptr<Queue<Packet> > txQueuen02 = ptr.Get<Queue<Packet> > ();
                                          txQueuen02->SetMaxSize( QueueSize (QueueSizeUnit::PACKETS, 10));
             tch.Install (d0d2);
...
//       ipv4.Assign (NetDevn2Attacker4);
   ...
   //setting up ingress node to communicate with the GTNET
       ...
       dev->SetAddress (Mac48Address ("00:00:00:5c:af:58"));
...
//        ipv4.Assign (NetDevDER4Egress);
        ...
        //Second Emulated interface Egress node
             std::string device2Name ("enp0s20u6"); //edit the name corresponding to the device name "enp0s20u6"
             ...
             //tch.Install (dev1);
             dev1->SetAddress (Mac48Address ("00:00:00:b2:8e:c8"));
...
if (dosEnabled){
    ...
    uint16_t port = 7001;   // Discard port (RFC 863)
      ...
      onOffapps.Start (Seconds (100.0));
       onOffapps.Stop (Seconds (150.0));

}


// Attacker sending a ICMPReroute message to route the traffic through it.
//Ptr<Ipv4> ipv4_n2 = n.Get(2)->GetObject<Ipv4>();
//Ptr<IpL4Protocol> prot = ipv4_n2->GetProtocol (Icmpv4L4Protocol::GetStaticProtocolNumber ());
//Ptr<Icmpv4L4Protocol> icmpv4 =prot->GetObject<Icmpv4L4Protocol> ();
//
//
//    Ptr<NetDevice> oif =ipv4_n2->GetNetDevice(2); //specify non-zero if bound to a source address
